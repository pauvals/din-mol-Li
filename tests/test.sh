...
if [[ "$tests" == *"ermak"* ]]; then
	...
	diff <(tail -n 1206 Li.xyz) ref.xyz > /dev/null && echo -e "\e[1;103mOk_Ermak\e[0m" || echo -e "\e[1;101mFAIL_Ermak\e[0m"
	cd ..
fi
...
if [[ "$tests" == *"gcmc"* ]]; then
  x=gcmc
	...
	diff <(tail -n 596 Li.xyz) ref.xyz > /dev/null && echo -e	"\e[1;103mOk_$x\e[0m" || echo -e "\e[1;101mFAIL_$x\e[0m"
	cd ..
fi
